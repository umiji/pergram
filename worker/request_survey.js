 ...
 *    `email` は NULL のまま残る。触れば、**それまで匿名だった押下がメールアドレスへ紐づく**
 *    （T-071 の最重要事項。表の CHECK 制約 `id IS NULL OR email IS NULL` が最後の砦になる）。
 * 🔒 **`id` を空文字 `''` で埋めてはならない。**（`id` が読めないときは送られてこないので、
 *    今の実装では起きない。将来「無ければ空文字」と書き換えないための注意である。）
 *    ⚠️ このとき危ないのは CHECK ではなく **UNIQUE の側**である。`id=''` の行は
 *    CHECK を通るが（`email` が NULL なので）、`id TEXT UNIQUE` により**表に1つしか
 *    存在できない。** 2人目以降の回答が ON CONFLICT(id) で1行目を上書きし、
 *    **別々の人の回答が1行に混ざる。**
