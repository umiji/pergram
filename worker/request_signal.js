 ...
 *
 * === `INSERT OR IGNORE` は送り直しの受け皿でもある 🔒（T-062） ===
 * ブラウザは**受け取ったことを確認できるまで、同じ id で送り直す**
 * （`pergram.request_signal_ack`）。以前は送信の成否を見ずに黙り込んでいたため、
 * 最初の1回が失敗したブラウザが二度と1行も残せなかった。
 * **したがって同じ id の2度目は「異常」ではなく通常の経路である。**
 * ここを `INSERT` に変えると送り直しが 503 になり、ブラウザは永久に確認を得られない。
 */
