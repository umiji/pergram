...
--    ただし**同じ人の行は同居しない** —— メールアドレスが送られた時点で、
--    匿名の行を引き取って1行にまとめる（下の T-072 の項を読むこと）。
--
...
--    この線を越えることまでは含んでいない。**
-- 🔒 **同じ人の行は1つにまとめる。まとめるときは必ず id を捨てる**（T-072）。
--    メールアドレスが送られた時点で、/api/waitlist が**匿名の行の回答を引き取り、
--    その行を消してから**メールアドレスの行へ書く。残るのは email を持つ1行だけで、
--    request_signal と突き合わせる鍵はどこにも保存されない。
--    **「1行にするために CHECK を外す」は解ではない** —— 外した瞬間に上の紐づけが起きる。
--    ⚠️ **T-071 の「同じ人が2行に分かれて残るのが正しい姿。『重複しているから』と
--    まとめないこと」は失効している**（PO 指摘、T-072 / 2026-09-09:「同じセッションで
--    成分アンケートの内容を送ったあとにメールアドレスを送ったらレコード2つに分かれるのは
--    流石にだめでしょ」）。2行のままだと同じ回答が両方の行に入り、
--    **「クレアチンを見たい人」を数えると同じ人を2回数えていた**
--    （docs/research/validation-plan.md の判定に直接効く）。
--    **失効したのは「だから行を2つに分ける」の部分だけで、すぐ上の
--    「同じ行に id と email を置かない」という結論そのものは生きている。**
--    正典は worker/waitlist.js の冒頭と docs/tasks/T-072.md の決定ログ。
-- 🔒 **各受け口は自分の鍵の列だけを書く。** /api/waitlist は email を、
--    /api/request-survey は id を書き、もう一方には触らない（NULL のまま残す）。
--    まとめるときも同じで、**/api/waitlist は id 列へ一切書かない**（消すだけ）。
--    メールの段が本文に添えてくる匿名の識別子（signal_id）は、**どの行を引き取るかを
--    指すためだけのもの**であり、保存されない。**保存列の id と同じ名前にしないこと。**
--    **もう一方を空文字 '' で埋めない。**
