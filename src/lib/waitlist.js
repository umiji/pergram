/**
 * 待機リストの登録処理。ホスティングに依存しない部分はすべてここに置く。
 *
 * アダプタは2つある。どちらもこのモジュールを呼ぶだけで、判断をここ以外に持たない。
 *   - functions/api/waitlist.js  Cloudflare Pages Functions
 *   - api/waitlist.js            Vercel Functions
 *
 * 使ってよいのは Web 標準の API だけ（fetch / crypto.subtle / btoa / atob）。
 * node: の import を足すと Cloudflare 側で動かなくなる。
 *
 * 保存先は2つ。どちらも任意で、環境変数が揃っているものにだけ書く。
 *   - Cloudflare D1（`env.DB`。Cloudflare でのみ束縛される）
 *   - Google スプレッドシート（サービスアカウント。どちらのホスティングでも使える）
...
// アクセストークンは isolate / インスタンス内で使い回す。毎リクエスト取り直さない
...
export async function upsertToSheet(config, record) {
  ...
  if (index >= 0) {
    ...
    await sheetsFetch(config, token, `/values/${encodeURIComponent(range)}?valueInputOption=RAW`, {
      method: 'PUT',
      body: JSON.stringify({
        range,
        majorDimension: 'ROWS',
...
/** Cloudflare の外（Vercel など）では束縛されない。文字列が入っていても D1 ではない */
const d1Binding = (env) =>
  env.DB && typeof env.DB === 'object' && typeof env.DB.prepare === 'function' ? env.DB : null;
...
/**
 * 検証と保存。HTTP に依存しないので、返すのは status と body だけ。
 * アダプタ側はこれをそのままレスポンスにする。
 */
export async function handleSubmission(payload, env) {
  const record = sanitizeSubmission(payload);
  if (!record) return { status: 400, body: { error: 'invalid_email' } };

  const db = d1Binding(env);
  const sheets = sheetsConfig(env);
  const targets = [];
  if (db) targets.push({ name: 'd1', run: () => upsertToD1(db, record) });
