 ...
 *    「毎回同じことを言っている」状態になった。**価格系は2つだけ**にしてある。
 *    増やすときは「価格以外」を足す。ネタの具体例は
 *    .claude/skills/x-post/reference/topics.md。
 *
 *    サービスの外の話（相場・制度・作る側の裏話）も入れてある。読み手は
 *    コスパ重視のトレーニーで、**うちのサービスの話より業界の話の方が面白い**。
 */
export const TOPIC_LEXICON = {
  単価の出し方: /1\s?[gｇ]あたり|単価|含有率で割|袋の(値段|価格)|実質|per\s?gram/i,
  価格の動き: /セール|プライムデー|ゾロ目|スーパーセール|取得日|取り直/,
  市場と相場: /値上げ|高騰|相場|原料|輸入|円安|関税|GLP-1|需給|価格改定/,
  表示の読み方: /栄養成分表示|100\s?[gｇ]あたり|1食あたり|無水|乾物|アミノ酸組成|原材料|表記|基準は/,
  作る側の裏話: /API|検索|商品名|絞り込|除外|読み取|自動で読|件中|件のうち|人力|手入力|下書き|名寄せ|JAN|レビュー|星/,
  ...
  単価という考え方: /ユニットプライス|単位価格|棚|スーパー|条例|ISO|他の(商品|ジャンル)/,
