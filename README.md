...
npm run x:feed                        # 過去投稿・配分の偏り・**次の1本のジャンル候補**
...
何を書くかは印象で決めない。`x:feed` が直近の投稿から配分（価格の話 3本に1本 /
うちの話 5本に1本 / URL を貼る投稿 10本に1本）を数え、上限を超えた族を外したうえで
**次の1本の候補**を出す。🔒 **サービスの告知は毎回しない。既定は「告知しない」。**
