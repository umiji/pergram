...
| `src/lib/x_feed.js` | 過去投稿フィードの読み取り・重複判定・**次に書くジャンルの選定**。RSS 2.0 / Atom / 二重エスケープのどれでも本文が取れること。読めないと重複チェックが**落ちずに素通りする**。⚠️ `TOPIC_LEXICON` が痩せるとアカウントが1つの話しかしなくなる。**投稿タイプの比率（①市場の発見25/②コスパ比較25/③あるある20/④データ15/⑤開発日記10/⑥告知5）と題材の上限（価格3本に1本・うちの話15%）が唯一の出所**。skill 側に数字を書き写さない |
...
| `.claude/skills/x-post/` | X 投稿文の作り方。戦略は `docs/Marketing/`、語彙は CLAUDE.md が勝つ。⚠️ 戦略文書の例文は禁止語（コスパ最強 / 個人開発 等）を含むので、そのまま使わない。**画像は明示的に頼まれたときだけ作る**。流行の型は `reference/formats.md`（寿命があるので古びたら書き直す）。人格は「めちゃくちゃプロテインに詳しい、コスパ意識の高い友達」（`reference/voice.md`）。投稿タイプと比率は `reference/post_types.md`、ツリーとクリフハンガーは `reference/threads.md`、題材は `reference/topics.md`（22ジャンル・5族）。**140字に収めるために不自然に削らない — 超えたらツリー**。🔒 **既定は「告知しない」** — 直接告知は20本に1本 |
