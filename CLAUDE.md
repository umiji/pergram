...
| `wrangler.toml` | Cloudflare のバインディングの唯一の出所。**このファイルがあるとダッシュボードの設定は無視される。**プレビュー用 D1 を本番と同じにしない |
...
| [docs/ops/deploy.md](docs/ops/deploy.md) | Cloudflare Pages + D1 のデプロイ手順。バインディングの出所は `wrangler.toml` ひとつ |
