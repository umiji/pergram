 ...
 *    受け入れテストがある。**実装より先に書いてあるので、未実装のうちは落ちるのが正しい。**
 *    落ちる 2 件には `PENDING`（一時的な skip）を付けて main の CI を赤にしないようにしてある。
 *    **T-017 の実装担当は、着手時にまず `PENDING` を外して RED を確認すること。**
 *    RED を見ずに実装を始めると、テストが何を要求しているのかを取り違える。
 *    実装が終わった時点で `PENDING` の定義ごと削除する。
 *    skip を付けたのはテスト担当であり、アサーションの削除も緩和もしていない。
 *
 * 🔒 未定義のカスタムプロパティを使わない。
...
// ⚠️ T-017 の実装が未着手である。PENDING を付けた 2 件は実装が入るまで必ず落ちる
//    受け入れテストであり、main の CI を赤にしたままにしないために一時的に skip してある。
//    実装担当は着手時に PENDING を外して RED を確認し、実装後に定義ごと削除すること。
const PENDING = { skip: 'T-017 実装待ち。実装担当は着手時にこの skip を外すこと' };

/* ---- 完了条件1: 主CTA の文字 -------------------------------------------- */

test('🔒 完了条件1: 主CTA (.btn--signal) の文字が背景に対して 4.5:1 以上', PENDING, async () => {
...
test('🔒 完了条件1b: 従CTA の面か境界がページ背景に対して 3:1 以上', PENDING, async () => {
  const { tokens, page, sheets } = await loadContext();
