...
// 2026-09-07 / T-051: LP の待機リストが段構造（要望 → アンケート → メール → 支援）へ
// 置き換わり、送信ボタンの属性の並びが変わった。**見張る中身は変わらない** ——
...
test('🔒 ヒーローから消えても意思表示の受け皿は残っている（ヘッダとページ下部フォーム）', () => {
  ...
  assert.ok(
    ...
    'ページ下部のフォームの送信ボタンがありません',
  );
  assert.ok(html.includes('id="waitlist"'), '意思表示のアンカー先がありません');
});
