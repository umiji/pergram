...
export async function runLpScript(html, options = {}) {
   ...
   *
   * 🔒 **守るべき線は「サーバへ送らないこと」であって、「localStorage に置かないこと」
   *    ではない。** `.claude/rules/pergram-compliance.md`「データ保護」はこう言っている ——
   *    「年齢・性別・服用中サプリは **localStorage / IndexedDB のみ**。サーバへ送信しない」。
   *    **localStorage は禁じられた置き場ではなく、指定された置き場である。**
   *    したがってテスト側で弾くのは、ここへ書く実装ではなく、
   *    **ここにあるものをサーバへ送る実装**である（`dom.fetchCalls` の本文を見る）。
   *
   * ⚠️ **以前ここには「置いてよいのは表示の状態と匿名の識別子だけ」と書いてあった。
   *    その文言は規約と逆を向いており、実際に1回、誤った設計判断を導いている**
   *    （T-070 / R-1。「送れなかったアンケートの回答を送り直すために手元へ控える」を
   *    規約違反と読み、失敗した回答を捨てる設計が正しいと判定された）。**戻さないこと。**
   *
   * 🔒 サーバへ送る前提のものを送るまで控える場合（出荷箱）は、
   *    **受領できた時点で消えること**を検査する（`tests/request_survey_outbox.test.js`）。
   *    控えが永続的な記録に化けるのが、この経路で唯一まずい壊れ方である。
   */
