...
test('🔒 帯の文の1区切りは 17 文字以内', () => {
  // design.md §4.8。320px で中身が使えるのは 254px。--size-sm 12.5px → 20 文字が限界
  //
  // ⚠️ 2026-09-08、主文の上限を 13 → 17 へ上げた。PO が指定した主文
  //    「いまはプロテインだけの β 版です。」が 17 文字だったため。
  //    **緩めたのではなく、320px で実測して安全を確かめてから上げた** ——
  //    「いまはプロテインだけの」(11) / 「β 版です。」(6) の2行に割れ、横溢れ0・孤立行0。
  //    390px 以上では1行に収まる。β と 版 の間は U+00A0（分割禁止の空白）で繋いであり、
  //    **これを通常の空白へ直すと 320px で「版です。」が孤立行になる。**
  const limits = {
    'request.lede': 17,
    'request.sub': 17,
    'request.micro': 17,
