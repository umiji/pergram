 ...
 *      → **2026-09-07 / T-051 で削除された。** この目印はもう使わない
 *
...
/* ====================================================================== */
/* 完了条件2: 待機リストの案内 → T-051 で削除された（検査も廃止）           */
/* ====================================================================== */
/*
 * T-050 の完了条件2は「待機リストの案内（`.waitlist-banner`）が製品リストより前に
 * 出ていること」だった。**T-051 でその案内ごと削除された**（PO 指摘: 製品一覧に
 * 意思表示の入口が2つ並び、重い対価の側が軽い側を打ち消していた。T-051 ## 変更範囲）。
 *
 * 案内が出ていないことの検査は tests/request_unify.test.js の
 * 「完了条件1 待機リストの案内ブロックが製品一覧から出力されない」が持っている。
 * ここへ戻すと、削除したものの存在を要求する検査になり、必ず落ちる。
 */
