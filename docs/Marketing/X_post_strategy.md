...
> ⚠️ この節は 2026年8月に公開されたアルゴリズムのコードで裏を取り直してある。
> 重みの一次資料は [X_algorithm_2026.md](X_algorithm_2026.md)。**数値が食い違ったらそちらが勝つ。**

1. **1投稿目にURLを貼らない**
   * URLは**「ツリーの2投稿目（自分のリプライ欄）」**に貼る。
   * ⚠️ ただし理由は「URLにペナルティがあるから」ではない。公開コードにその定数は無く、
     リンクを開く行動はむしろ +0.2 で評価される。1投稿目を空けるのは**機会費用**
     ——1投稿目を会話（リプライ 5.0／相互フォロー相手なら 20.0）に使う方が取り分が大きいから。
...
3. **画像のキャプチャ・デモ動画を1枚目に添付する**
   * 実際の比較画面や「1gあたり〇円」と並んでいるUIのスクショ/動画があると、読まれやすくなります。
   * ⚠️ ただし画像そのものの重みは小さい（拡大・再生とも 0.05）。効くのは
     **読み飛ばされないこと**（滞在しなかった -0.02 を避けること）であって、
     画像を貼ればスコアが上がるわけではない。**毎回付けなくてよい。**
