...
| 4 | 🟢 **測る側の道具は、この組織にほぼ揃っている。** `org-tokens.py` が既に会話記録（JSONL）を読み、タスク別・担当別・モデル別のトークンと**往復回数**を出している（⚠️ この調査の時点では単位が誤っていた。T-074 で修正済み） | **足りないのは「毎回同じ条件の課題」だけ**である |
...
| 指標 | 取れるか | 出所 |
|---|---|---|
| **実 API 往復** | 🟢 **取れている**（T-074 で修正済み） | 台帳の **`往復回数`** 列。⚠️ **2026-09-13 より前の `返答回数` 列は応答の行数で、実往復の約1.9倍だった** |
